...
ANAHTAR_KELIME_YANITLARI = {
    ...
    "nasılsın":
    ["İyiyim, teşekkürler!", "Harika! Sen nasılsın?", "Çalışıyorum, sen?"],
    "ne yapıyorsun": [
        "Seninle konuşuyorum :)", "Kod satırları arasında dolaşıyorum!",
        "Sana yanıt vermekle meşgulüm."
    ],
    "günaydın": [
        "Günaydın! Harika bir gün seni bekliyor!", "Güneş gibi parlıyorsun!",
        "Bugün harika geçsin!"
    ],
    "iyi geceler": [
        "İyi geceler, tatlı rüyalar!", "Uykun bol, kabusun hiç olmasın!",
        "Yarın görüşürüz!"
    ],
    "teşekkürler": [
    ...
    "seni seviyorum": [
        "Ben de seni seviyorum 💖", "Aww, çok naziksin!",
        "Kalp kalbe karşı derler 😄"
    ],
    "görüşürüz":
    ["Görüşmek üzere!", "Kendine iyi bak!", "Yakında tekrar konuşalım!"],
    "bot musun": [
        "Yok ya ben gizli bir ajanım... 😏", "Evet, ama kalbim var!",
        "Botum ama dost canlısıyım :)"
    ],
    "neden çalışmıyorsun": [
        "Belki de uykum var 😴", "Bir hata olabilir, tekrar dener misin?",
        "Şu an biraz meşgulüm 😅"
    ],
    "sıkıldım": [
        "Hadi birlikte oyun oynayalım!", "Sana bir fıkra anlatayım mı?",
        "Sıkılmak mı? Ben buradayım!"
    ],
    "şaka yap": [
        "Geçen gün elektrikle konuştum, 'akım' oldu!",
        "Matematikçiye sormuşlar: 'Yalnız mısın?' Cevap: 'Sinüsle kosinüs var!'",
        "Bilgisayar çökmüş, neden? Çünkü 'Windows' açık kalmış! 😅"
    ],
    "müzik öner": [
        "Lo-fi mi seversin yoksa rock mı?", "Bugünlük biraz jazz öneririm 🎷",
        "Hadi nostalji: Barış Manço - Gülpembe 💿"
    ],
    "film öner": [
        "Inception tam senlik!", "Bugünlük komedi: The Grand Budapest Hotel!",
        "Biraz gerilim: Shutter Island nasıl?"
    ]
}
...
async def on_message(message):
    ...
    if not OTO_YANIT_KANALLAR or message.channel.id in OTO_YANIT_KANALLAR:
        ...
        if bot.user.mentioned_in(message) and not message.mention_everyone:
            user_message = re.sub(rf"<@!?{bot.user.id}>", "",
                                  message.content).strip()

            if not user_message:
                await message.channel.send(
                    f"Merhaba {message.author.mention}! Nasıl yardımcı olabilirim?"
                )
            else:
...
async def yardım(ctx):
    embed = discord.Embed(title="Bot Komutları",
                          description="İşte kullanabileceğiniz komutlar:",
                          color=discord.Color.blue())
    embed.add_field(name="!merhaba",
                    value="Bot size selam verir",
                    inline=False)
    embed.add_field(name="!sor [soru]",
                    value="Yapay zekaya soru sorun",
                    inline=False)
    embed.add_field(name="@Bot [mesaj]",
                    value="Botu etiketleyerek de soru sorabilirsiniz",
                    inline=False)
    embed.set_footer(
        text="Daha fazla yardım için geliştiriciyle iletişime geçin")
...
async def çal(ctx, *, arama):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(f"ytsearch:{arama}",
...
async def yardımcı(ctx):
    ...
    embed.add_field(name="@Bot [mesaj]",
                    value="Botu etiketleyerek de soru sorabilirsiniz",
                    inline=False)
    embed.add_field(name="!oyunsaatim",
                    value="Steam'deki oyun saatlerini gösterecek",
                    inline=False)

    embed.add_field(name="Ses Komutları:",
                    value="🎶 Sesli komutlar hakkında bilgi",
                    inline=False)
    embed.add_field(name="!çal [arama]",
                    value="YouTube'dan bir şarkı arar ve ses kanalında çalar",
                    inline=False)
    embed.add_field(name="!dur", value="Çalan şarkıyı durdurur", inline=False)
    embed.add_field(name="!çık",
                    value="Bot ses kanalından çıkar",
                    inline=False)

    embed.add_field(name="NSFW Komutları:",
                    value="🔞 Bu komutlar, NSFW içeriklere yöneliktir",
                    inline=False)
    embed.add_field(name="!waifu",
                    value="Waifu görseli gönderir",
                    inline=False)
    ...
    embed.add_field(name="!blowjob",
                    value="Blowjob görseli gönderir",
                    inline=False)
